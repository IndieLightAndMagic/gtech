...
GTech::Shader::Shader(ShaderSource * shaderSource, GLenum shaderType){
    ...
    try {
        ...
        m_state=GTech::Shader::SHADER_STATE_CREATED;
    }
    catch (int error){

        std::cout << "ERROR::GTech::SHADER::SHADER_NOT_SUCCESFULLY_CREATED" << std::endl;
        return;
    
    }
    if ( shaderSource != 0x00 && m_state == GTech::Shader::SHADER_STATE_CREATED) {
        try {
            ...
            m_state = SHADER_STATE_COMPILED;
                ...
                std::cout<< "ERROR::GTech::SHADER::COMPILATION_FAILED\n" << infoLog << std::endl << shaderSource[0]() << std::endl;
...
const GLuint GTech::Shader::operator()(){
    return m_shader;
}

GTech::Shader::ShaderState GTech::Shader::state(){ return m_state; }
...
unsigned int shaderTest(GLenum shaderType){
    ...
    if (s.state()==GTech::Shader::SHADER_STATE_CREATED){
